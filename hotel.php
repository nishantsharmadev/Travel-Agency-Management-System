...
<div id= "modal" class="popup">  
        ...
        <table id="pkg">
            ...
            <?php 
              ...
              //   $hotel= $_SESSION['package'];
              
                  $sql= "SELECT * FROM hotels WHERE location= '$hotel';";
                  $results = mysqli_query($db, $sql);
                  if (mysqli_num_rows($results) > 0)
                  {
                      while($row= mysqli_fetch_assoc($results))
                      {
                          echo  "<tr><td>". $row["hotelid"] ."</td><td>" . $row["name"] ."</td><td>". $row["price"] ."</td><td>" .$row["rating"] ."</td><td>" .$row["location"] . "</td></tr>";
                      }
                      echo "</table>";  
            ...
            <form class="input" method="post" action="index3.php">
               <input type="text" name="hotel" required>
                <button>Enter Hotel Name</button>
            </form>
